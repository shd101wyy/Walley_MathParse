...
bool checkWhetherSameVerbExistedFromStrList(char **str_list,char *var_name){
    //printf("check");
    //printf("#### checkWhetherSameVarNameExistsFromFile ####\n");
    bool has_same_var_name=FALSE;
    int row=0;
    while (str_list[row] != NULL) {
        char *arr=str_list[row];
        if(find(arr,":")==-1){
            row++;
            continue;
        }
...
bool isFunctionFromVar(char *input_str){
    //printf("#### isFunction ###\n");
    //printf("input_str %s\n",input_str);
    bool is_function=TRUE;
    input_str = removeAheadSpace(input_str);
    if(count_str(input_str,"(")==0 || count_str(input_str, ")")==0){
       ...
       // printf("function name-> |%s|\n",function_name);
        //if (checkWhetherSameVarNameExistsFromFile("__walley_function__.wy", function_name))
        int row=0;
        int length_of_function=atoi(FUNCTION[0]);
        while (row<length_of_function) {
           // printf("->%s\n",FUNCTION[row]);
            int index_of_maohao=find(FUNCTION[row],":");
            if (index_of_maohao==-1) {
                row++;
                continue;
            }
            char *temp=substr(FUNCTION[row],0, index_of_maohao);
...
char *println_from_var_file(char *var_in_file_name, char *input_str){
    ...
    char *output=malloc(sizeof(char)*((int)strlen(input)+2));
    strcpy(output,input);
    strcat(output,"\n");
    output[(int)strlen(input)+1]=0;
    return output;
}
