...
void Walley_Run_For_Appointed_Var(struct VAR **struct_var, struct VAR **struct_settings, char ***save_to_file, char* existing_file, char ***FUNCTION_functions, char* input_str) {
   ...
   // printf("######### Run Function Walley_Run_For_Appointed_Var ######\n");
    ...
    //printf("input_str after removing space |%s|\n",input_str);
            ...
            if (current_space % 4 != 0) {
               ...
               // printf("Finish_define_FUNCTION\n");
                ...
                if ((find(removeAheadSpace(input_str), "exp")==0&&input_str[(int)strlen(input_str)-1]==':')|| now_writting_expression==TRUE) {
                    //printf("input_str |%s|, space %d, current_space %d\n",input_str,space,current_space);
...
char *Walley_Translate_To_Function_From_Var(char *input_str, char *best_match_sentence, struct VAR **struct_var){
    ...
    if (index_of_gang==-1) {
        ...
        print("it should be in format function|expression \n");
        exit(0);
    }
...
char *Walley_Substitute_Var_And_Function_Return_Value_From_Var(char* input_str,struct VAR **struct_var, char ***FUNCTION_functions){//, char *file_function_name){
    ...
    /*
    if (strcmp(input_str, "y[0].var_value+\":\"")==0) {
    ...
    */
    ...
    else if(strcmp("string",input_str)==0){
    ...
    while (count_str_not_in_string(input_str, "<@")!=0) {
        ...
        begin = find_not_in_string(input_str, "<@");
        //end = find_from_index_not_in_string(input_str, ">", begin + 1);
        int next_begin=find_from_index_not_in_string(input_str, "<@", begin+1);
        if (next_begin==-1 || next_begin>end) {
            begin=begin;
        }
        else{
            begin=next_begin;
            next_begin=find_from_index_not_in_string(input_str, "<@", begin+1);
            while (TRUE) {
                if (next_begin==-1 || next_begin>end) {
                    begin=begin;
                    break;
        ...
        if (find_function) {
            ...
            if (it_is_slice==FALSE) {
                ...
                //#####################  println  ###################
                    ...
                    char* output = Walley_Println(struct_var,FUNCTION_functions, strInBrackets(input_str));
                    printf("%s", output);
                    return_value="None";
                }//#####################  print  ###################
                else if (find(input_str, "walley_print(") == 0) {
                    char* output = Walley_Print(struct_var,FUNCTION_functions, strInBrackets(input_str));
                    printf("%s", output);
                    ////puts(output);
                    return_value="None";
                }
                else if(find(input_str,"walley_run_str(")==0){
   ...
   // printf("############################\n");
    ...
    // ##############  Function  ##############################
       ...
       // bool has_same_function_name = checkWhetherSameFunctionNameExistsFromVar(func_name);
        ...
        /*
        // I delete those codes on Dec 4
        //if (has_same_function_name) {
            //printf("Has Same Function Name\n");
            // I delete the code below :             changeFunctionFromItsInitialOne(func_name, func_param_str);
        ...
        */
        ...
        //if (NOW_IN_DEF==TRUE) {
        //    FUNCTIONS_IN_DEF=listAppendOneElement(FUNCTIONS_IN_DEF, toString(func_name));
        //}
